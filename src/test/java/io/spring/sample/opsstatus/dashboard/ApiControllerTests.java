...
class ApiControllerTests {
	...
	void incidentWithNoSuchIncident() {
		...
		assertThat(mvc.get().uri("/api/incidents/42").accept(MediaType.APPLICATION_PROBLEM_JSON))
			.hasStatus(HttpStatus.NOT_FOUND)
			.hasContentTypeCompatibleWith(MediaType.APPLICATION_PROBLEM_JSON)
			.bodyJson()
			.withResourceLoadClass(getClass())
			.isEqualTo("missing-incident-42.json");
	}

	@Test
	void incidentWithNoSuchIncidentErrorView() {
		when(this.incidentRepository.findById(42L)).thenReturn(Optional.empty());
		assertThat(mvc.get().uri("/api/incidents/42").accept(MediaType.TEXT_HTML)).hasStatus(HttpStatus.NOT_FOUND)
